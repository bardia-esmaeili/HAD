...
for RANK in $(seq 0 "$((WORLD_SIZE - 1))"); do
  JOB_ID="$(sbatch --parsable \
    ...
    --export=ALL,PROJECT_ROOT="${PROJECT_ROOT}",DATASET="${DATASET}",DATA_ROOT="${DATA_ROOT}",OUTPUT_ROOT="${OUTPUT_ROOT}",SCENES_FILE="${SCENES_FILE}",LVSM_ROOT="${LVSM_ROOT}",LVSM_CKPT_PATH="${LVSM_CKPT_PATH}",SPARSE_VIEW="${SPARSE_VIEW}",VIEW_FUSION="${VIEW_FUSION}",USE_LVSM="${USE_LVSM}",USE_ORACLE="${USE_ORACLE}",USE_DIFIX_DELTA="${USE_DIFIX_DELTA}",DATA_FACTOR="${DATA_FACTOR}",MAX_STEPS="${MAX_STEPS}",TARGET_SAMPLE_STEP="${TARGET_SAMPLE_STEP}",MIPNERF_SPLIT_ROOT="${MIPNERF_SPLIT_ROOT}",SPLIT_JSON="${SPLIT_JSON}",UNCERTAINTY_MASK_THRESHOLD="${UNCERTAINTY_MASK_THRESHOLD}",UNCERTAINTY_MASK_MODE="${UNCERTAINTY_MASK_MODE}",UNCERTAINTY_MASK_TEMPERATURE="${UNCERTAINTY_MASK_TEMPERATURE}" \
    "${SCRIPT_DIR}/run_had_eval_dataset_shard.sh" "${RANK}" "${WORLD_SIZE}")"
  JOB_IDS+=("${JOB_ID}")
  echo "submitted DreamAware3D ${DATASET} eval rank=${RANK}/${WORLD_SIZE} use_lvsm=${USE_LVSM} use_oracle=${USE_ORACLE} use_difix_delta=${USE_DIFIX_DELTA} job_id=${JOB_ID}"
done
